...
log_banner()  {
    echo -e "${CYAN}${BOLD}"
    echo "=========================================================================="
    echo "   FreeSWITCH Security & Fail2Ban Setup v2.1.0 (patched)"
...
# ─────────────────────────────────────────────────────────────────────────
# NEW: verify FreeSWITCH is actually reaching the journal — this is the
# #1 reason bans silently never happen.
# ─────────────────────────────────────────────────────────────────────────
FS_JOURNAL_OK=0
verify_freeswitch_logging() {
    log_step "Verifying FreeSWITCH -> systemd journal logging"

    if ! systemctl list-units --type=service 2>/dev/null | grep -q freeswitch; then
        log_warn "FreeSWITCH service not found — skipping journal check (jails will simply stay idle until it exists)."
        return 0
    fi

    local sample
    sample=$(journalctl -u freeswitch.service --no-pager -n 200 2>/dev/null || true)

    if [[ -z "$sample" ]]; then
        log_error "No journal entries at all for freeswitch.service."
        log_warn  "fail2ban's 'backend = systemd' will NEVER see auth failures until this is fixed."
        log_warn  "Fix: make sure FreeSWITCH's console/syslog logging is directed to the journal:"
        log_warn  "  1) In freeswitch.xml (or autoload_configs/console.conf.xml / syslog.conf.xml),"
        log_warn  "     enable syslog logging, OR"
        log_warn  "  2) Ensure the systemd unit does NOT redirect stdout/stderr away from the journal"
        log_warn  "     (no 'StandardOutput=null' in the unit file), then:"
        log_warn  "       systemctl daemon-reload && systemctl restart freeswitch"
        log_warn  "  3) Re-check with: journalctl -u freeswitch.service -f"
        FS_JOURNAL_OK=0
        return 0
    fi

    if echo "$sample" | grep -qiE "sofia_reg\.c|SIP auth"; then
        log_success "FreeSWITCH SIP registration log lines found in journal — systemd backend will work."
        FS_JOURNAL_OK=1
    else
        log_warn "Journal has FreeSWITCH entries, but no SIP auth-related lines yet (may just mean no recent"
        log_warn "auth attempts). Jails will start working as soon as such lines appear. If bans still don't"
        log_warn "happen after a real failed-login test, check the exact log line format with:"
        log_warn "  journalctl -u freeswitch.service | grep -i 'auth failure'"
        log_warn "and compare it against filter.d/freeswitch-auth.conf — sofia_reg.c line numbers/wording"
        log_warn "can differ across FreeSWITCH versions."
        FS_JOURNAL_OK=1
    fi
}
...
# Fail2Ban Filter: FreeSWITCH SIP Authentication Failures (systemd backend)
[INCLUDES]
before = common.conf
[Definition]
_daemon = freeswitch
failregex = ^\s*\[WARNING\] sofia_reg\.c:\d+ SIP auth failure \(REGISTER\) on sofia profile \'.+\' for \[.*\] from ip <HOST>\s*$
...
ignoreregex = ^\s*\[.*\] .*from ip 127\.\d+\.\d+\.\d+\s*$
...
[Definition]
_daemon = freeswitch
failregex = ^\s*\[WARNING\].*User-Agent:.*sipvicious.*from.*<HOST>.*$
...
[Definition]
_daemon = freeswitch
failregex = ^\s*\[WARNING\] sofia_reg\.c:\d+ SIP auth challenge \(REGISTER\) on sofia profile \'.+\' for \[.*\] from ip <HOST>\s*$
...
[Definition]
_daemon = freeswitch
failregex = ^\s*\[WARNING\].*sofia\.c:\d+.*Invalid SIP.*<HOST>.*$
...
JAILS
    ...
    log_success "Fail2Ban jails configured (with dashboard logging wired in)."
}
...
print_summary() {
    ...
    echo ""
    if [[ "$FS_JOURNAL_OK" -eq 0 ]]; then
        echo -e "  ${RED}${BOLD}WARNING: FreeSWITCH is NOT confirmed to be logging to the systemd journal.${NC}"
        echo -e "  ${RED}Jails will NOT ban anyone until this is fixed — see the warnings printed above.${NC}"
    else
        echo "  FreeSWITCH journal logging: confirmed working."
    fi
    echo ""
